...
param(
    ...
    # Shared parameters
    [string]$ApplicationName = 'MvcMusicStore',
    ...
    [string]$S3Bucket,
    [string]$S3Folder,
    [string]$MainBinary = 'MvcMusicStore',

    # ECS-specific parameters
    [string]$StackName = "AWSTransform-Deploy-App-Stack-MvcMusicStore-4d76634d",
...
ECS Parameters:
    ...
    -OperatingSystem       : Container operating system
...
function Get-ParameterOverrides {
    ...
    if ($DeploymentType -eq "ECS") {
        ...
        if ($OperatingSystem) {
            ...
            $paramOverrides += "OperatingSystem=$OperatingSystem"
        }
        if ($EnvironmentVariables) {
            Log "Environment Variables: $EnvironmentVariables"
            $paramOverrides += "EnvironmentVariables=$EnvironmentVariables"
        }
        if ($ECSTaskCount) {
